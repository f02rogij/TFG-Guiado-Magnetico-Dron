%% ================================================================
%  dron_escalera3D.m   (3a Aproximacion)
%  ----------------------------------------------------------------
%  OBJETIVO: demostrar que la distribucion en ESCALERA (railes
%  antiparalelos + travesanos) mejora el seguimiento de la autopista
%  magnetica respecto al cable recto: los travesanos rompen la simetria
%  de traslacion y dan sensibilidad de gradiente TAMBIEN en el along.
%  Correccion por GRADIENTES en los 3 ejes (perp, along, z).
%  CASOS:  control (sin correccion) | con correccion | con perturbaciones
%  La distribucion NO esta optimizada para sensibilidad VERTICAL, asi que
%  el eje z queda como mejora pendiente (trabajo futuro). Tambien queda
%  pendiente la actitud (campo espacialmente rugoso). Coste J real.
%% ================================================================
clear; close all; tic;
SEED = 42;
[script_dir, prog] = fileparts(mfilename('fullpath'));
if isempty(script_dir), script_dir = pwd; prog = 'dron_escalera3D'; end
...
nav.b_g=2.5e-6; nav.b_az=5e-4; nav.k_obs=0.8;

%% ── Geometria ESCALERA EN ESQUINA ──────────────────────────────────
Lc=30; w=4; P=8;                 % tramo, semianchura calzada, paso travesanos
...
%% ── Trayectoria por el eje de la calzada ────────────────────────────
wp=[-2,0,5,0; Lc,0,5,0; Lc,0,5,0; Lc,Lc+2,5,0];
v_avg=0.3; t_dwell=5;
...
fprintf('=== Escalera (tramos %d m, calzada %d m, P=%d) | v=%.3f | Tfin=%.1f s ===\n\n',Lc,2*w,P,v_avg,Tfin);
...
%% ── Simulaciones finales: 3 casos ───────────────────────────────────
fprintf('\nSimulando casos...\n');
S_ctrl=run_sim(G,p,t_sim,rd_full,Ts,N_sim,x0_drone,precomp,mag,wires,nav,false,false);
S_corr=run_sim(G,p,t_sim,rd_full,Ts,N_sim,x0_drone,precomp,mag,wires,nav,false,true);
S_pert=run_sim(G,p,t_sim,rd_full,Ts,N_sim,x0_drone,precomp,mag,wires,nav,true, true);
...
plot_convergencia(hi,hf,fval,G,'Cuckoo (escalera 3D)',P_('evolucion_cuckoo_escalera3D.png'));
plot_results(S_ctrl,t_sim,rd_full,Tfin,'Control (sin correccion) | Escalera 3D',     P_('caso_control_escalera3D.png'));
plot_results(S_corr,t_sim,rd_full,Tfin,'Con correccion 3D | Escalera',               P_('caso_correccion_escalera3D.png'));
plot_results(S_pert,t_sim,rd_full,Tfin,'Con perturbaciones + correccion | Escalera', P_('caso_perturbaciones_escalera3D.png'));
plot_observabilidad(S_corr,t_sim,precomp,t_wp,'Observabilidad 3D — Escalera con correccion',P_('observabilidad_escalera3D.png'));
plot_3d(S_ctrl,t_sim,rd_full,t_wp,wp,'control',           wires,P_('trayectoria_3D_control_escalera3D.png'));
plot_3d(S_corr,t_sim,rd_full,t_wp,wp,'con correccion',    wires,P_('trayectoria_3D_correccion_escalera3D.png'));
plot_3d(S_pert,t_sim,rd_full,t_wp,wp,'con perturbaciones',wires,P_('trayectoria_3D_perturbaciones_escalera3D.png'));
...
function [psi_wire,foot]=nearest_segment(r_obs,wires)
    r_obs=r_obs(:);dist_min=inf;psi_wire=0;foot=r_obs;nd=wires.track;
    for s=1:(size(nd,1)-1)
        p1=nd(s,:)';p2=nd(s+1,:)';lv=p2-p1;L=norm(lv); if L<1e-12,continue;end
...
function plot_3d(Sa,t_sim,rd,t_wp,wp,etiqueta,wires,fp) %#ok<INUSD>
    ...
    % limites con MARGEN para que el hilo exterior no toque el borde de los ejes
    allp=[rd(:,1:3); Sa.X(:,[1 3 5])];
    for c=1:numel(wires.cond), allp=[allp; wires.cond{c}.nodes]; end %#ok<AGROW>
    mn=min(allp,[],1); mx=max(allp,[],1); mrg=max(0.05*max(mx-mn),0.6);
    daspect(ax,[1 1 1]);
    xlim(ax,[mn(1)-mrg mx(1)+mrg]); ylim(ax,[mn(2)-mrg mx(2)+mrg]); zlim(ax,[mn(3)-mrg mx(3)+mrg]);
    xlabel(ax,'x[m]');ylabel(ax,'y[m]');zlabel(ax,'z[m]');
    %legend('Referencia','Trayectoria','Inicio','Fin','Location','best');   % leyenda comentada
    title(sprintf('Trayectoria 3D escalera — %s',etiqueta),'FontWeight','bold');drawnow;
    try exportgraphics(fig,fp,'Resolution',300);catch,print(fig,fp,'-dpng','-r300');end;close(fig);
end
